...
std::optional<LRESULT> PlatformWindow::handleMouseEvent(const MouseEvent& event) {
	...
	case WM_MOUSEMOVE:
		if (view != _enteredView) {
			if (_enteredView) _enteredView->onMouseLeave();
			_enteredView = view;
			if (_enteredView) _enteredView->onMouseEnter();
		}
		view->onMouseMove(event);
		break;
	case WM_MOUSELEAVE:
		if (_enteredView) {
			_enteredView->onMouseLeave();
...
std::optional<LRESULT> PlatformWindow::handleNativeEvent(const MSG& event) {
	auto isMouseEvent = [](UINT message) -> bool {
